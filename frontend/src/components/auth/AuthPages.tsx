...
import { ShieldCheck, Mail, ArrowRight, KeyRound, CheckCircle2, Lock, Loader2 } from 'lucide-react';
...
export const LoginPage: React.FC<AuthProps> = ({ onLoginSuccess, onNavigateGuest }) => {
  ...
  const handleGithubLogin = async () => {
    ...
    try {
      ...
      if (res?.error) {
        setError(res.error.message || 'Failed to initiate GitHub authentication');
        setIsLoading(false);
      } else if (res?.data?.url) {
        window.location.href = res.data.url;
      }
    } catch (err: any) {
      setError(err.message || 'Failed to initiate GitHub authentication');
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (res?.error) {
        setError(res.error.message || 'Failed to initiate Google authentication');
        setIsLoading(false);
      } else if (res?.data?.url) {
        window.location.href = res.data.url;
      }
    ...
    <div className="min-h-screen bg-[#0a0b0d] text-[#f5f7fa] flex items-center justify-center p-4 selection:bg-[#7c5cff]/20">
      ...
      <div className="relative w-full max-w-md bg-[#191c22] border border-[#24272f] rounded-2xl shadow-2xl overflow-hidden p-8 flex flex-col gap-6 animate-fadeIn">
        ...
        {view !== 'forgot' && (
          <div className="flex flex-col gap-2.5">
            <button
              ...
              disabled={isLoading}
              className="flex items-center justify-center gap-3 w-full bg-[#131519] border border-[#24272f] hover:border-[#8a74ff]/50 hover:bg-[#20242c] text-[#f5f7fa] font-medium py-2.5 px-4 rounded-xl text-sm transition-all duration-200 cursor-pointer shadow-sm active:scale-[0.98] disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={isLoading}
              className="flex items-center justify-center gap-3 w-full bg-[#131519] border border-[#24272f] hover:border-[#3b82f6]/50 hover:bg-[#20242c] text-[#f5f7fa] font-medium py-2.5 px-4 rounded-xl text-sm transition-all duration-200 cursor-pointer shadow-sm active:scale-[0.98] disabled:opacity-50"
            >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <Button variant="primary" size="md" type="submit" isLoading={isLoading} className="w-full mt-2">
